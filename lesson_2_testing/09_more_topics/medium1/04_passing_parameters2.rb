...
If we want to assign the entire array to a single variable, we can do
that:
...
as an argument. The method should yield the contents of the array to a
block, which should assign your block variables in such a way that it
ignores the first two elements, and groups all remaining elements as
a raptors array.
=end
...
raptors(birds){ |_ ,_, *raptors| p raptors }
